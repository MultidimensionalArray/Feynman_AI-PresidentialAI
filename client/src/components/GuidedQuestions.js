...
const GuidedQuestions = ({ questions, onComplete, onBack, topic }) => {
  ...
  const restartGuidedSection = (reason) => {
  ...
  const nextQuestion = () => {
    ...
    const vague = nonAnswer || isVagueAnswer(answer);
    ...
    if (nextNonAnswerCount >= 3) {
      restartGuidedSection("Let's reset this part and try again with more complete answers.");
      return;
    }

    if (totalAnswered >= 3 && nextVagueCount / totalAnswered > 0.5) {
      restartGuidedSection("Let's restart this section and aim for clearer, more specific answers.");
      return;
    }
